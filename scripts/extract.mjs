...
function cleanFragment($, $el) {
  ...
  // LeadConnector iframe resizers), not to WordPress — dropping them leaves the
  // embeds unsized. They keep their original absolute URLs: the iframe they size is
  // served by the same third-party host, so mirroring the loader would not remove a
  // dependency, it would only add a copy that goes stale. Inline scripts are
